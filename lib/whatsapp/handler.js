 ...
 * @description Manipulador de mensagens do WhatsApp
 * 
 * Este módulo gerencia o processamento de mensagens recebidas do WhatsApp,
 * coordenando o fluxo de interação com o usuário, geração de carta e
...
const state = require('../redis/state');
...
async function processIncomingMessage(userPhoneNumber, messageText) {
  try {
    ...
    if (!userState || !userState.state || userState.state === 'NEW') {
