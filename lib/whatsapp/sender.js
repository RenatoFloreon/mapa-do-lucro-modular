 ...
 */
async function sendWhatsAppMessage(phoneNumber, messageBlocks, attempt = 1, maxAttempts = 3) {
  ...
  for (let i = 0; i < messageBlocks.length; i++) {
    ...
    try {
      ...
      if (attempt < maxAttempts && (error.code === 'ETIMEDOUT' || error.code === 'ECONNABORTED' || error.code === 'ECONNRESET' || error.name === 'AbortError')) {
...
// Exportar a função com o nome correto (W maiúsculo em WhatsApp)
module.exports = {
  sendWhatsAppMessage
};
